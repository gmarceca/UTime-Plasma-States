...
def run(args, gpu_mon):
    ...
    elif hparams["build"]["TransferLearning"] or hparams["build"]["TransferLearning2"]:
        from utime.models.model_init import prepare_for_transfer_learning
        parameter_file = prepare_for_transfer_learning(project_dir=project_dir, path=hparams["build"]["TL_model"],
                                                        logger=logger)
    else:
        parameter_file = args.initialize_from  # most often is None
    
    if parameter_file:
        print("LOADING MODEL from {}".format(parameter_file))
