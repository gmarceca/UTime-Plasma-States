...
def prepare_for_transfer_learning(project_dir, path, logger=None):
    """
    Args:
    ...
    Returns:
        A path to the model weight files to use for transfer learning.
        Will be None if no model files were found
    """
    from MultiPlanarUNet.utils import get_last_model
    model_path, epoch = get_last_model(os.path.join(project_dir, path))
    return model_path
